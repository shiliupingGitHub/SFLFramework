...
namespace Jitter.LinearMath
{
    ...
    public static class JConvexHull
    {
        ...
        public static int[] Build(List<JVector> pointCloud, Approximation factor)
        {
            ...
            {
                // [0,PI]
                Fix64 theta = JMath.Pi / (steps - 1) * thetaIndex;
                Fix64 sinTheta = (Fix64)Fix64.Sin(theta);
                Fix64 cosTheta = (Fix64)Fix64.Cos(theta);

                for (int phiIndex = 0; phiIndex < steps; phiIndex++)
                {
                    // [-PI,PI]
                    Fix64 phi = (2.0f * JMath.Pi) / (steps - 0) * phiIndex - JMath.Pi;
                    Fix64 sinPhi = (Fix64)Fix64.Sin(phi);
                    Fix64 cosPhi = (Fix64)Fix64.Cos(phi);
        ...
        {
            int index = 0;
            Fix64 current = Fix64.MinValue;

            JVector point; Fix64 value;
