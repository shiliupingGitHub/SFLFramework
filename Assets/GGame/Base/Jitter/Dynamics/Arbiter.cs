...
namespace Jitter.Dynamics
{
    ...
    public class Arbiter
    {
        ...
        public Contact AddContact(JVector point1, JVector point2, JVector normal, Fix64 penetration, 
        ...
        private void ReplaceContact(ref JVector point1, ref JVector point2, ref JVector n, Fix64 p, int index,
            ContactSettings contactSettings)
        {
        ...
        private int GetCacheEntry(ref JVector realRelPos1, Fix64 contactBreakThreshold)
        {
            Fix64 shortestDist = contactBreakThreshold * contactBreakThreshold;
            int size = contactList.Count;
            int nearestPoint = -1;
            for (int i = 0; i < size; i++)
            {
                JVector diffA; JVector.Subtract(ref contactList[i].relativePos1,ref realRelPos1,out diffA);
                Fix64 distToManiPoint = diffA.LengthSquared();
        ...
        private int SortCachedPoints(ref JVector realRelPos1, Fix64 pen)
        {
            //calculate 4 possible cases areas, and take biggest area
            //also need to keep 'deepest'

            int maxPenetrationIndex = -1;
            Fix64 maxPenetration = pen;
            for (int i = 0; i < 4; i++)
            {
            ...
            Fix64 res0 = 0, res1 = 0, res2 = 0, res3 = 0;
        ...
        internal static int MaxAxis(Fix64 x, Fix64 y, Fix64 z, Fix64 w)
        {
            int maxIndex = -1;
            Fix64 maxVal = Fix64.MinValue;
