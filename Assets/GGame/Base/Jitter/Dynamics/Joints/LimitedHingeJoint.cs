...
namespace Jitter.Dynamics.Joints
{
    ...
    public class LimitedHingeJoint : Joint
    {
        ...
        public LimitedHingeJoint(World world, RigidBody body1, RigidBody body2, JVector position, JVector hingeAxis,
            Fix64 hingeFwdAngle, Fix64 hingeBckAngle)
            : base(world)
        {
            ...
            Fix64 angleToMiddle = 0.5f * (hingeFwdAngle - hingeBckAngle);
            ...
            Fix64 hingeHalfAngle = 0.5f * (hingeFwdAngle + hingeBckAngle);
            Fix64 allowedDistance = len * 2.0f * (Fix64)Fix64.Sin(hingeHalfAngle * 0.5f / 360.0f * 2.0f * Fix64.PI);
