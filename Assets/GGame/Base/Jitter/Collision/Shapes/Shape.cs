    ...
    public abstract class Shape : ISupportMappable
    {
        ...
        public virtual void MakeHull(ref List<JVector> triangleList, int generationThreshold)
        {
            ...
            while (activeTriList.Count > 0)
            {
                ...
                Fix64 d1 = (p2 - p1).LengthSquared();
                Fix64 d2 = (p3 - p2).LengthSquared();
                Fix64 d3 = (p1 - p3).LengthSquared();

                if (Math.Max(Math.Max((float)d1, (float)d2), (float)d3) > distanceThreshold && tri.generation < generationThreshold)
                {
        ...
        /// <returns></returns>
        #region  public static Fix64 CalculateMassInertia(Shape shape, out JVector centerOfMass, out JMatrix inertia)
        public static Fix64 CalculateMassInertia(Shape shape, out JVector centerOfMass,
            out JMatrix inertia)
        {
            Fix64 mass = 0.0f;
            ...
            Fix64 a = 1.0f / 60.0f, b = 1.0f / 120.0f;
            ...
            for (int i = 0; i < hullTriangles.Count; i += 3)
            {
                ...
                Fix64 tetrahedronMass = (1.0f / 6.0f) * detA;
