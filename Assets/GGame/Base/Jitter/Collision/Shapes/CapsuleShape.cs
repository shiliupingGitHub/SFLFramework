...
namespace Jitter.Collision.Shapes
{
    ...
    public class CapsuleShape : Shape
    {
        ...
        {
            Fix64 massSphere = (3.0f / 4.0f) * JMath.Pi * radius * radius * radius;
            Fix64 massCylinder = JMath.Pi * radius * radius * length;
        ...
        {
            Fix64 r = (Fix64)Fix64.Sqrt(direction.X * direction.X + direction.Z * direction.Z);

            if (Fix64.Abs(direction.Y) > 0.0f)
            {
                JVector dir; JVector.Normalize(ref direction, out dir);
                JVector.Multiply(ref dir, radius, out result);
                result.Y += Fix64.Sign(direction.Y) * 0.5f * length;              
            }
            else if (r > 0.0f)
            {
