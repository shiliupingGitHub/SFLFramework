...
namespace Jitter.Collision
{
    ...
    public sealed class GJKCollide
    {
        ...
        private static void SupportMapTransformed(ISupportMappable support, ref JMatrix orientation, ref JVector position, ref JVector direction, out JVector result)
        {
            ...
            Fix64 x = ((result.X * orientation.M11) + (result.Y * orientation.M21)) + (result.Z * orientation.M31);
            Fix64 y = ((result.X * orientation.M12) + (result.Y * orientation.M22)) + (result.Z * orientation.M32);
            Fix64 z = ((result.X * orientation.M13) + (result.Y * orientation.M23)) + (result.Z * orientation.M33);
        ...
        public static bool Pointcast(ISupportMappable support, ref JMatrix orientation,ref JVector position,ref JVector point)
        {
            ...
            Fix64 dist = v.LengthSquared();
            Fix64 epsilon = 0.0001f;
    ...
    //    }
        ...
        public static bool Raycast(ISupportMappable support, ref JMatrix orientation, ref JMatrix invOrientation,
            ref JVector position,ref JVector origin,ref JVector direction, out Fix64 fraction, out JVector normal)
        {
            VoronoiSimplexSolver simplexSolver = simplexSolverPool.GetNew();
            simplexSolver.Reset();
        ...
        */
        ...
        private class VoronoiSimplexSolver
        {
                ...
                if (_needsUpdate)
                {
                    ...
                    switch (NumVertices)
                    {
                        ...
                        case 2:
                            ...
                            JVector diff = from * (-1);
                            JVector v = to - from;
                            Fix64 t = JVector.Dot(v, diff);

                            if (t > 0)
                            {
                                Fix64 dotVV = v.LengthSquared();
                                if (t < dotVV)
                                {
                                    t /= dotVV;
                                    diff -= t * v;
                ...
                ref SubSimplexClosestResult result)
            {
                ...
                JVector ac = c - a;
                JVector ap = p - a;
                Fix64 d1 = JVector.Dot(ab, ap);
                Fix64 d2 = JVector.Dot(ac, ap);
                if (d1 <= 0f && d2 <= 0f)
                {
                ...
                JVector bp = p - b;
                Fix64 d3 = JVector.Dot(ab, bp);
                Fix64 d4 = JVector.Dot(ac, bp);
                if (d3 >= 0f && d4 <= d3)
                {
                ...
                JVector cp = p - c;
                Fix64 d5 = JVector.Dot(ab, cp);
                Fix64 d6 = JVector.Dot(ac, cp);
                if (d6 >= 0f && d5 <= d6)
                {
            ...
            public int PointOutsideOfPlane(JVector p, JVector a, JVector b, JVector c, JVector d)
            {
                ...
                Fix64 signp = JVector.Dot(p - a, normal); // [AP AB AC]
                Fix64 signd = JVector.Dot(d - a, normal); // [AD AB AC]
                ...
                ref SubSimplexClosestResult finalResult)
            {
                ...
                if (pointOutsideABC != 0)
                {
                    ...
                    Fix64 sqDist = ((JVector)(q - p)).LengthSquared();
