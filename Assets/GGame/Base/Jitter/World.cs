...
namespace Jitter
{
    ...
    public class World
    {
        public delegate void WorldStep(Fix64 timestep);
        ...
        public class WorldEvents
        {
            ...
            internal void RaiseWorldPreStep(Fix64 timestep)
            {
                if (PreStep != null) PreStep(timestep);
            }

            internal void RaiseWorldPostStep(Fix64 timestep)
            {
        ...
        private Fix64 inactiveAngularThresholdSq = 0.1f;
        private Fix64 inactiveLinearThresholdSq = 0.1f;
        private Fix64 deactivationTime = 2f;

        private Fix64 angularDamping = 0.85f;
        private Fix64 linearDamping = 0.85f;

        private int contactIterations = 10;
        private int smallIterations = 4;
        private Fix64 timestep = 0.0f;
        ...
        /// The default value is 0.85</param>
        public void SetDampingFactors(Fix64 angularDamping, Fix64 linearDamping)
        {
        ...
        public void SetInactivityThreshold(Fix64 angularVelocity, Fix64 linearVelocity, Fix64 time)
        {
        ...
        private Fix64 currentLinearDampFactor = 1.0f;
        private Fix64 currentAngularDampFactor = 1.0f;
        ...
        public void Step(Fix64 timestep, bool multithread)
        {
            ...
            // Calculate this
            currentAngularDampFactor = Fix64.Powf(angularDamping, timestep);
            currentLinearDampFactor = Fix64.Powf(linearDamping, timestep);
        ...
        private Fix64 accumulatedTime = 0.0f;
        ...
        public void Step(Fix64 totalTime, bool multithread, Fix64 timestep, int maxSteps)
        {
            int counter = 0;
        ...
        private void UpdateArbiterContacts(Arbiter arbiter)
        {
            ...
            for (int i = arbiter.contactList.Count - 1; i >= 0; i--)
            {
                ...
                else
                {
                    ...
                    Fix64 distance = JVector.Dot(ref diff, ref c.normal);
        ...
        private void IntegrateCallback(object obj)
        {
            ...
            if (!(body.isParticle))
            {
                ...
                //exponential map
                JVector axis;
                Fix64 angle = body.angularVelocity.Length();
                ...
                else
                {
                    ...
                    JVector.Multiply(ref body.angularVelocity, ((Fix64)Fix64.Sin(0.5f * angle * timestep) / angle), out axis);
                }

                JQuaternion dorn = new JQuaternion(axis.X, axis.Y, axis.Z, (Fix64)Fix64.Cos(angle * timestep * 0.5f));
        ...
        private void CollisionDetected(RigidBody body1, RigidBody body2, JVector point1, JVector point2, JVector normal, Fix64 penetration)
        {
